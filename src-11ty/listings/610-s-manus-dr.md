---
address: 610 S Manus Dr
city: Dallas
state: TX
zip: "75224"
status: sold
status_badge: Sold
has_detail_page: true
page_slot: 2
photos_slug: 610-s-manus-dr
photo_count: 62
price: "$685,000"
price_per_sqft: "$190"
beds: "4"
baths: "3.5"
sqft: "3,605"
bedrooms: 4 Bedrooms
bathrooms: 3.5 Bathrooms
squarefoot: "3605"
lot: "0.71"
year_built: "1955"
tagline: "Waterfront living on a private pond — dock, pool & timeless character"
gallery_title: Tour the Home
gallery_desc: Step inside 610 S Manus Dr — from the sunlit interiors to the water's edge.
reel_title: Walk through every room.
reel_desc: A room-by-room tour of 610 S Manus Dr — sunlit interiors, hardwood floors, bay windows, and out to the pond's edge.
tour_title: 3D Walkthrough
tour_desc: Explore every room at your own pace with the interactive Zillow 3D Home tour.
about_title: A Rare Waterfront Canvas
agent_title: Thinking About Selling Yours?
agent_desc: I'm Kelly Cypret with Nolen Rodgers Realty. This one has found its new owner — but if you love this pocket of Dallas, or you're wondering what your own home would bring today, call or text me. I'd love to help.
kelly_comments: Sold!
phone_display: "(469) 812-8100"
phone_dial: "+14698128100"
email: "Kelly@NRRTsold.com"
video_id: "Eic4JsdXFKo"
video_url: "https://www.youtube.com/embed/Eic4JsdXFKo?autoplay=1&mute=1&loop=1&playlist=Eic4JsdXFKo&controls=1&playsinline=1&rel=0&modestbranding=1"
tour_url: "https://www.zillow.com/view-imx/631dd0ab-315d-4e7d-9434-e58d599b3fcf?setAttribution=mls&wl=true&initialViewType=pano&utm_source=dashboard"
hero_image: /assets/images/properties/610-s-manus-dr/full/62.webp
external_url: ""
featured: true
order: 1
hero_slides:
  - src: /assets/images/properties/610-s-manus-dr/full/40.webp
    pos: "center center"
  - src: /assets/images/properties/610-s-manus-dr/full/46.webp
    pos: "center center"
  - src: /assets/images/properties/610-s-manus-dr/full/38.webp
    pos: "center center"
  - src: /assets/images/properties/610-s-manus-dr/full/06.webp
    pos: "center center"
  - src: /assets/images/properties/610-s-manus-dr/full/16.webp
    pos: "center center"
  - src: /assets/images/properties/610-s-manus-dr/full/01.webp
    pos: "center center"
  - src: /assets/images/properties/610-s-manus-dr/full/24.webp
    pos: "center center"
  - src: /assets/images/properties/610-s-manus-dr/full/27.webp
    pos: "center center"
  - src: /assets/images/properties/610-s-manus-dr/full/62.webp
    pos: "center center"
  - src: /assets/images/properties/610-s-manus-dr/full/56.webp
    pos: "center center"
gallery:
  - src: /assets/images/properties/610-s-manus-dr/thumbs/40.webp
    full: /assets/images/properties/610-s-manus-dr/full/40.webp
    cls: "wide tall"
    caption: "Lakeside balcony view"
  - src: /assets/images/properties/610-s-manus-dr/thumbs/46.webp
    full: /assets/images/properties/610-s-manus-dr/full/46.webp
    cls: ""
    caption: "Pool & lake"
  - src: /assets/images/properties/610-s-manus-dr/thumbs/38.webp
    full: /assets/images/properties/610-s-manus-dr/full/38.webp
    cls: ""
    caption: "Great room to the water"
  - src: /assets/images/properties/610-s-manus-dr/thumbs/06.webp
    full: /assets/images/properties/610-s-manus-dr/full/06.webp
    cls: ""
    caption: "Entry foyer & staircase"
  - src: /assets/images/properties/610-s-manus-dr/thumbs/16.webp
    full: /assets/images/properties/610-s-manus-dr/full/16.webp
    cls: ""
    caption: "Kitchen"
  - src: /assets/images/properties/610-s-manus-dr/thumbs/01.webp
    full: /assets/images/properties/610-s-manus-dr/full/01.webp
    cls: "wide"
    caption: "Front exterior"
  - src: /assets/images/properties/610-s-manus-dr/thumbs/24.webp
    full: /assets/images/properties/610-s-manus-dr/full/24.webp
    cls: ""
    caption: "Primary bedroom"
  - src: /assets/images/properties/610-s-manus-dr/thumbs/27.webp
    full: /assets/images/properties/610-s-manus-dr/full/27.webp
    cls: ""
    caption: "Primary bath"
  - src: /assets/images/properties/610-s-manus-dr/thumbs/62.webp
    full: /assets/images/properties/610-s-manus-dr/full/62.webp
    cls: "wide"
    caption: "Pond & Dallas skyline"
  - src: /assets/images/properties/610-s-manus-dr/thumbs/56.webp
    full: /assets/images/properties/610-s-manus-dr/full/56.webp
    cls: "wide"
    caption: "Overhead aerial"
highlights:
  - icon: tint
    label: "Waterfront on a private pond"
  - icon: anchor
    label: "Private dock at the water's edge"
  - icon: life-ring
    label: "Sparkling pool"
  - icon: building-o
    label: "Balcony overlooking the water"
  - icon: sun-o
    label: "Bay windows & natural light"
  - icon: square-o
    label: "Hardwood flooring throughout"
  - icon: tree
    label: "Mature trees on a 0.71-acre lot"
  - icon: road
    label: "Minutes to I-35, shopping & dining"
reel_features:
  - icon: sun-o
    label: "Expansive windows & natural light"
  - icon: tint
    label: "Private pond + dock"
  - icon: square-o
    label: "Hardwood floors throughout"
  - icon: building-o
    label: "Balcony with water views"
facts_interior:
  - label: Bedrooms
    value: "4"
  - label: Bathrooms
    value: "3.5"
  - label: Living area
    value: "3,605 sqft"
  - label: Cooling
    value: Central
  - label: Heating
    value: "Yes"
  - label: Fireplace
    value: "Yes"
  - label: Flooring
    value: Hardwood
facts_property:
  - label: Lot size
    value: "0.71 Acres"
  - label: Garage
    value: "2 — Attached"
  - label: Exterior
    value: Brick
  - label: Waterfront
    value: Private pond
  - label: Outdoor
    value: "Pool · Dock · Balcony"
  - label: Parcel
    value: "00000467359000000"
facts_construction:
  - label: Home type
    value: Single Family
  - label: Year built
    value: "1955"
  - label: Structure
    value: Frame
  - label: Foundation
    value: Piers
  - label: Roof
    value: Composition
  - label: County
    value: Dallas
---

Nestled on a beautifully landscaped lot in an established Dallas neighborhood, this unique waterfront property offers endless potential and picturesque views. Situated on a large pond with a private dock, you'll enjoy a peaceful setting rarely found in the city.

The home features 4 bedrooms, hardwood flooring throughout much of the home, expansive windows that fill the space with natural light, and charming bay windows that showcase the scenic surroundings. Outdoor living is a standout — a sparkling pool, a spacious balcony overlooking the water, and mature trees that frame the property.

Conveniently located near I-35, shopping, and dining, 610 S Manus blends tranquility with accessibility. Ready for a new owner's vision, it's a rare opportunity to renovate and create something truly special — an exceptional lot, water views, a private dock, a pool, and timeless character all in one place.
